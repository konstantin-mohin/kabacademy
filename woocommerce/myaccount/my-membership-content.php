...
<?php
    ...
    <div class="table-responsive">
        <table class="shop_table shop_table_responsive my_account_orders my_account_memberships my_membership_content table table-blue">
            ...
            <tr>
				 ...
				 *
				 */
				$my_membership_content_columns = (array) apply_filters( 'wc_memberships_members_area_my_membership_content_column_names',
					array(
...
//						'membership-content-excerpt'    => esc_html__( 'Excerpt', 'woocommerce-memberships' ),
						'membership-content-actions'    => wc_memberships_get_members_area_page_links( $customer_membership->get_plan(),
							'my-membership-content', $restricted_content ),
					), $user_id );
			...
			<?php foreach ( $restricted_content->posts as $member_post ) : ?>
				...
				$view_start_time  = wc_memberships_adjust_date_by_timezone( wc_memberships_get_user_access_start_time( $user_id,
					'view', array( 'post' => $member_post->ID ) ), 'timestamp', wc_timezone_string() );
                ...
                <tr class="membership-content">
					<?php foreach ( $my_membership_content_columns as $column_id => $column_header ) : ?>
						...
						<?php elseif ( 'membership-content-accessible' === $column_id ) : ?>
                            ...
                            <td class="membership-content-accessible table-text-blue"
								...
								<?php else : ?>
                                    <time datetime="<?php echo date( 'Y-m-d H:i:s', $view_start_time ); ?>"
                                          title="<?php echo esc_attr( $view_start_time ); ?>"><?php echo date_i18n( wc_date_format(),
						...
						<?php elseif ( 'membership-content-excerpt' === $column_id ) : ?>
                            ...
                            <!--                            <td class="membership-content-excerpt"-->
                            <!--                                data-title="--><?php //echo esc_attr( $column_header ); ?><!--">-->
                            <!--								--><?php //if ( empty( $member_post->post_excerpt ) ) : ?>
                            <!--									--><?php //echo wp_kses_post( wp_trim_words( strip_shortcodes( $member_post->post_content ),
//										20 ) ); ?>
                            <!--								--><?php //else : ?>
						...
						<?php elseif ( 'membership-content-actions' === $column_id ) : ?>
                            ...
                            <td class="membership-content-actions order-actions table-text-big table-text-blue"
                                data-title="<?php echo esc_attr( $column_header ); ?>">
								<?php echo wc_memberships_get_members_area_action_links( 'my-membership-content',
									$customer_membership, $member_post ); ?>
                            </td>

						<?php else : ?>

                            <td class="<?php echo esc_attr( $column_id ); ?> table-text-big table-text-blue"
                                data-title="<?php echo esc_attr( $column_header ); ?>">
								<?php do_action( 'wc_memberships_members_area_my_membership_content_column_' . $column_id,
									$member_post ); ?>
                            </td>
			...
			<?php $tfoot = wc_memberships_get_members_area_page_links( $customer_membership->get_plan(),
				'my-membership-content', $restricted_content ); ?>
